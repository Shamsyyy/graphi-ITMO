//
// Created by Daurchik on 22.03.2020.
//
...
void Image::read() {
    if (!this->in) {
        throw("Input file is not defined\n");
    }
...
void Image::write() {
    if (!this->out) {
        throw("Output file is not defined\n");
    }
...
void Image::horizontal_display() { // 2) зеркальное отражание по горизонтале
    if (this->mode[1] == '5')
        for (size_t i = 0; i < this->h; i++)
...
void Image::rotate(bool type) { // поворот на 90
    // 0 - clock wise
